...
//            }
    ...
    ArmState copy (ArmState a)
    {
        ...
        for (int n=0; n<numNodes; n++) {
            Node aNode = a.nodes.get (n);
            Node bNode = b.nodes.get (n);
            bNode.x = aNode.x;
            bNode.y = aNode.y;
        }
        return b;
	...
	// Now handle the case of Node 1, which is at one of
	// the intersection points of the circle from 0, and
	// the circle from Node 2.
	Node zeroNode = b.nodes.get(0);
	Node oneNode = b.nodes.get(1);
	Node twoNode = b.nodes.get(2);
    ...
    double distance (Node n, Point2D.Double p)
    {
